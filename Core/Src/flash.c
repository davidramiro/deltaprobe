...
FlashStatus saveToFlash(void) {
  ...
  if (cycles == num_cycles && threshold == sensor_threshold &&
      checksum == packedChecksum(cycles, threshold)) {
    return FLASH_OK;
  }
  ...
  status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, CHECKSUM_MEM_ADDR,
